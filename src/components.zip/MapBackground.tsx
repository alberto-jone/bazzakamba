...
export const MapBackground: React.FC<MapBackgroundProps> = ({ 
  ...
  const parks = [
    "M 220 420 L 320 440 L 310 550 L 200 520 Z", 
    "M 50 200 C 80 200, 100 100, 50 50 L 0 50 L 0 200 Z", 
    "M 300 250 C 350 250, 380 150, 320 100 L 280 120 Z" 
  ];
  ...
  useEffect(() => {
    switch (rideStatus) {
      ...
      case 'searching':
        ...
        break;
      case 'pickup':
        setBaseViewBox({ x: 50, y: 350, w: 250, h: 400 }); 
        break;
      case 'arrived':
        setBaseViewBox({ x: 100, y: 300, w: 200, h: 200 }); 
        break;
      case 'traveling':
        setBaseViewBox({ x: 100, y: 50, w: 300, h: 450 }); 
        break;
      case 'completed':
        setBaseViewBox({ x: 250, y: 0, w: 150, h: 150 }); 
        break;
      default:
  ...
  return (
    <div className="absolute inset-0 z-0 bg-[#E5E7EB] overflow-hidden font-sans group select-none">
      <style>{`
        .map-transition { transition: viewBox 1.5s cubic-bezier(0.25, 0.1, 0.25, 1); }
        @keyframes followPickupPath { 0% { offset-distance: 0%; } 100% { offset-distance: 100%; } }
        @keyframes followTripPath { 0% { offset-distance: 0%; } 100% { offset-distance: 100%; } }
        .vehicle-pickup { offset-path: path("${pickupPathData}"); animation: followPickupPath 10s linear forwards; offset-rotate: auto 0deg; }
        .vehicle-trip { offset-path: path("${tripPathData}"); animation: followTripPath 14s linear forwards; offset-rotate: auto 0deg; }
        .pulse-ring { animation: ping 2s cubic-bezier(0, 0, 0.2, 1) infinite; }
        @keyframes ping { 75%, 100% { transform: scale(2); opacity: 0; } }
      `}</style>

      <div className="absolute right-4 top-32 z-50 flex flex-col gap-2 shadow-xl">
        <button onClick={handleZoomIn} className="bg-white p-3 rounded-t-lg hover:bg-gray-50 text-gray-700 border-b border-gray-200">
          <Plus className="w-6 h-6" />
        </button>
        <button onClick={handleZoomOut} className="bg-white p-3 rounded-b-lg hover:bg-gray-50 text-gray-700">
          <Minus className="w-6 h-6" />
        </button>
      </div>

      <svg className="w-full h-full absolute inset-0 map-transition" viewBox={finalViewBox} preserveAspectRatio="xMidYMid slice">
        <rect x="-200" y="-200" width="1200" height="1600" fill="#F3F4F6" />
        <path d={riverPath} fill="#BFDBFE" stroke="#93C5FD" strokeWidth="2" />
        {parks.map((d, i) => <path key={i} d={d} fill="#D1FAE5" stroke="#A7F3D0" strokeWidth="1" />)}
        <g stroke="white" strokeWidth="12" fill="none" strokeLinecap="round">{secondaryRoads.map((d, i) => <path key={i} d={d} />)}</g>
        <g stroke="#E5E7EB" strokeWidth="10" fill="none" strokeLinecap="round">{secondaryRoads.map((d, i) => <path key={i} d={d} />)}</g>
        ...
        {rideStatus === 'pickup' && <path d={pickupPathData} stroke="black" strokeWidth="8" fill="none" strokeLinecap="round" strokeLinejoin="round" opacity="0.8" />}
        {rideStatus === 'traveling' && <path d={tripPathData} stroke="black" strokeWidth="8" fill="none" strokeLinecap="round" strokeLinejoin="round" opacity="0.8" />}
        ...
        {rideStatus === 'pickup' && <g className="vehicle-pickup"><g transform="rotate(90)"><VehicleIcon type={vehicleType} color={vehicleColor} /></g></g>}
        {rideStatus === 'arrived' && <g transform="translate(200, 400) rotate(45)"><VehicleIcon type={vehicleType} color={vehicleColor} /></g>}
        {rideStatus === 'traveling' && <g className="vehicle-trip"><g transform="rotate(90)"><VehicleIcon type={vehicleType} color={vehicleColor} /></g></g>}
        {rideStatus === 'completed' && <g transform="translate(350, 50) rotate(0)"><VehicleIcon type={vehicleType} color={vehicleColor} /></g>}
      </svg>
    </div>
  );
};
...
const VehicleIcon = ({ type, color }: { type: string, color: string }) => {
  ...
  return (
    <g transform={isMoto ? "scale(0.7)" : "scale(0.9)"}>
      ...
      {!isMoto && (
        <>
            <path d="M -10 -12 L 10 -12 L 10 2 L -10 2 Z" fill="#111827" opacity="0.9" />
            <path d="M -10 24 L 10 24 L 10 14 L -10 14 Z" fill="#111827" opacity="0.8" />
            <rect x="-11" y="2" width="22" height="12" fill={color} opacity="0.9" />
        </>
      )}
      {isMoto && (
          <>
            <rect x="-4" y="-15" width="8" height="10" fill="#111827" rx="2" />
            <circle cx="0" cy="10" r="5" fill="#111827" />
            <rect x="-10" y="-18" width="20" height="2" fill="black" />
          </>
      )}
