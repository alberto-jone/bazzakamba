  ...
  return (
    <div className="h-full w-full bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 p-6 space-y-4 overflow-y-auto">
        {RIDE_OPTIONS.map((ride) => (
          <div 
            ...
            style={selectedRide.id === ride.id ? { borderColor: ride.color, backgroundColor: ride.color + '0D' } : {}}
            className={`relative p-4 rounded-2xl border-2 transition-all cursor-pointer flex items-center gap-4 ${selectedRide.id === ride.id ? 'shadow-xl scale-[1.02]' : 'border-transparent bg-white shadow-sm hover:shadow-md opacity-80'}`}
          >
            ...
            <div className="text-right">
               <span className="block font-bold text-lg whitespace-nowrap" style={{ color: ride.color }}>Kz {ride.price.toLocaleString('pt-AO')}</span>
            </div>
            {selectedRide.id === ride.id && (
                <div className="absolute top-4 right-4">
                    <div className="w-4 h-4 rounded-full border-2 border-white shadow-sm" style={{ backgroundColor: ride.color }}></div>
                </div>
            )}
          </div>
        ))}
      ...
      <div className="p-6 bg-white border-t border-gray-100">
        <button onClick={onConfirm} className="w-full bg-[#007FF0] hover:bg-[#0066CC] text-white font-bold py-4 rounded-xl shadow-lg active:scale-[0.98] transition-transform">
