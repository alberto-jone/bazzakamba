...
export const HomeScreen: React.FC<HomeScreenProps> = ({ 
  ...
  const hasDashboardAccess = isAdmin || isDriver;
    ...
    <div className="h-full w-full relative flex flex-col">
      ...
      {isMenuOpen && (
        <div className="absolute inset-0 z-50 bg-black/50 animate-in fade-in backdrop-blur-sm">
          <div className="h-full w-72 bg-white shadow-2xl animate-in slide-in-from-left duration-300 flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 p-4 space-y-3 overflow-y-auto">
               {hasDashboardAccess && (
                 <div className="pb-2 border-b border-gray-100 mb-2">
                   ...
                   <button onClick={() => onNavigateTo('admin')} className="w-full flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-[#007FF0]/10 text-gray-900 font-bold transition-colors border border-gray-200 hover:border-[#007FF0]/30 group">
               ...
               {isAdmin && (
                <button onClick={handleExportReport} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 text-gray-700 font-medium transition-colors">
                  <FileDown className="w-5 h-5 text-gray-600" />
                  Exportar Relatório CSV
                </button>
               )}
               <button onClick={() => onNavigateTo('prototype_feedback')} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 text-gray-700 font-medium transition-colors">
                 <MessageSquare className="w-5 h-5 text-yellow-500" />
                 Avaliar Protótipo
               </button>
            </div>
            <div className="p-4 border-t border-gray-100 bg-gray-50">
               <button onClick={onLogout} className="w-full flex items-center gap-3 p-3 rounded-xl bg-white border border-gray-200 hover:bg-red-50 hover:border-red-100 text-red-600 font-medium transition-colors shadow-sm">
                 <LogOut className="w-5 h-5" />
                 Sair da Conta
               </button>
               <div className="text-center mt-4 text-[10px] text-gray-400 font-medium">Bazza Kamba v1.1.0 (RBAC)</div>
            </div>
          </div>
      ...
      <div className="absolute top-14 left-4 right-4 z-20 flex flex-col gap-2">
        <button onClick={() => setIsMenuOpen(true)} className="absolute -top-10 left-0 p-2.5 bg-white rounded-full shadow-md hover:bg-gray-50 transition-transform active:scale-95 border border-gray-100">
           <Menu className="w-6 h-6 text-gray-900" />
           {hasDashboardAccess && <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 rounded-full border border-white"></span>}
        </button>
        ...
        <div className="bg-gray-900 p-3 rounded-lg shadow-xl flex items-center gap-3 relative border border-gray-800 transition-all focus-within:ring-2 focus-within:ring-[#007FF0]">
          ...
          <div className="flex-1">
            ...
            <input type="text" value={destination} onChange={(e) => handleSearch(e.target.value)} placeholder="Digite o destino (ex: Açucareira)" className="w-full font-bold text-sm text-white placeholder-gray-500 bg-transparent outline-none" />
          </div>
          {destination && (
            <button onClick={() => { setDestination(''); setIsValidSelection(false); }} className="p-1">
        ...
        {showSuggestions && suggestions.length > 0 && (
          <div className="bg-white rounded-lg shadow-2xl overflow-hidden border border-gray-100 mt-1 max-h-48 overflow-y-auto animate-in fade-in slide-in-from-top-2">
            {suggestions.map((item, idx) => (
              <button key={idx} onClick={() => selectSuggestion(item)} className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 border-b border-gray-50 last:border-none flex items-center gap-2 font-medium">
      ...
      <div className="mt-auto z-10 bg-white rounded-t-3xl shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.2)] p-5 pb-8 space-y-4">
        ...
        <div className="grid grid-cols-4 gap-2">
             {RIDE_OPTIONS.map(ride => {
                 ...
                 return (
                   <button key={ride.id} onClick={() => onSelectRide(ride)} style={isSelected ? { backgroundColor: ride.color + '1A', borderColor: ride.color } : {}} className={`flex flex-col items-center gap-2 p-2 rounded-xl transition-all duration-200 border ${isSelected ? 'scale-105 shadow-sm' : 'border-transparent hover:bg-gray-50 opacity-70 hover:opacity-100'}`}>
                      ...
                      <div className="flex flex-col items-center">
                        <span className={`text-[10px] font-bold leading-tight ${isSelected ? 'text-black' : 'text-gray-500'}`}>{ride.name.split(' ')[0]}</span>
                        <span className="text-[10px] font-bold" style={{ color: isSelected ? ride.color : '#9CA3AF' }}>Kz {ride.price}</span>
                      </div>
                   </button>
                 )
             })}
        </div>
        <button onClick={onNext} disabled={!isValidSelection} className={`w-full font-bold py-4 rounded-xl shadow-lg active:scale-[0.98] transition-all text-white text-lg flex items-center justify-center gap-2 ${isValidSelection ? 'bg-[#007FF0] hover:bg-[#0066CC]' : 'bg-gray-300 cursor-not-allowed'}`}>
          {isValidSelection ? <>Solicitar {selectedRide.name}</> : 'Selecione um Destino'}
        </button>
      </div>
    </div>
  );
