...
import { LogOut, Download, Users, Activity, Star, Lock, AlertCircle, UserPlus, X, Car, MapPin, TrendingUp } from 'lucide-react';
...
export const AdminScreen: React.FC<AdminScreenProps> = ({ onBack, onGoToMap }) => {
  ...
  const avgAppRating = db.getAverageAppRating();
  const totalPassengers = users.filter(u => u.role === 'passenger').length;
  const totalDrivers = users.filter(u => u.role === 'driver').length;
  const totalRides = db.getTotalRidesCount();
          ...
          {isAdmin && (
            <>
              <div className="bg-white/10 p-4 rounded-2xl backdrop-blur-sm border border-white/20">
                ...
                <div className="text-[10px] opacity-70 mt-1">{totalPassengers} pass. / {totalDrivers} mot.</div>
              </div>
              <div className="bg-white/10 p-4 rounded-2xl backdrop-blur-sm border border-white/20">
                <div className="flex items-center gap-2 mb-2">
                  <Activity className="w-4 h-4 text-white" />
                  <span className="text-[10px] font-bold uppercase opacity-80">Viagens</span>
                </div>
              ...
              <div className="bg-white/10 p-4 rounded-2xl backdrop-blur-sm border border-white/20">
                ...
                <div className="text-2xl font-bold">{avgAppRating}</div>
              </div>
              <div className="bg-white/10 p-4 rounded-2xl backdrop-blur-sm border border-white/20">
                <div className="flex items-center gap-2 mb-2">
                ...
                <div className="text-2xl font-bold">{avgDriverRating}</div>
              </div>
            </>
          )}          {isDriver && (
            <>
