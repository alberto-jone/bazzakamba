...
export const db = {
  ...
  // --- NOVO: FUNÇÃO PARA SINCRONIZAR DASHBOARD (Adicionado aqui) ---
  syncDashboardData: async () => {
    ...
    try {
      ...
      // Busca dados das 4 tabelas em paralelo
      const [usersRes, ridesRes, driverFbRes, appFbRes] = await Promise.all([
        supabase.from('users').select('*'),
        supabase.from('ride_history').select('*'),
        supabase.from('driver_feedback').select('*'),
        supabase.from('app_feedback').select('*')
      ]);

      // Atualiza o LocalStorage com os dados frescos para garantir consistência
      if (usersRes.data) localStorage.setItem(DB_KEYS.USERS, JSON.stringify(usersRes.data));
      if (ridesRes.data) localStorage.setItem(DB_KEYS.RIDE_HISTORY, JSON.stringify(ridesRes.data));
      if (driverFbRes.data) localStorage.setItem(DB_KEYS.FEEDBACK_DRIVER, JSON.stringify(driverFbRes.data));
      if (appFbRes.data) localStorage.setItem(DB_KEYS.FEEDBACK_APP, JSON.stringify(appFbRes.data));
      ...
      return {
        users: usersRes.data || [],
        rides: ridesRes.data || [],
        driverFeedbacks: driverFbRes.data || [],
        appFeedbacks: appFbRes.data || []
      };
    } catch (error) {
  ...
  saveRide: (ride: Omit<Ride, 'id'>) => {
    ...
    if (supabase) {
      supabase
        ...
        .insert(newRide)
        .then(({ error, data }: any) => {
          if (error) console.error('Erro ao salvar viagem no Supabase:', error);
          else console.log('Viagem salva no Supabase:', data);
        });
    }
    return newRide;
  },
  ...
  getDriverAverageRating: (driverId?: string): number => {
    const feedbacks = db.getDriverFeedback();
    if (feedbacks.length === 0) return 0;
    const total = feedbacks.reduce((acc, f) => acc + f.rideRating, 0);
    return Math.round((total / feedbacks.length) * 10) / 10;
  },
